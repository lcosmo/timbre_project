    ...
    <body>
        ...
        <table>
            <thead>
                <tr>
                    ...
                    <th>Surname</th>
                    <th>Email</th>
                    <th>Nr of evaluated  Links</th>
                    <th>Search Aim</th>
                    <th>Selected categories of information</th>
                    <th>Nr of clicked Links</th>
        ...
        <?php
            $ustable = " SELECT u.*, sum(if(r.id IS NULL, 0, 1)) as rated " .
             ...
             $query = " SELECT sess.*, p.category_name, p.sequential_order, p.relevance_order FROM ".
                      " ($query) sess, searchpreferences p ".
                      " WHERE sess.id=p.usersessions_id AND sess.email!='admin'".
                      " ORDER BY sess.order, sess.email, p.sequential_order";
...
//                     " ";
            ...
            while($row = $result->fetch_assoc())
            {
                ...
                $rated = $row['rated'];
                $category = $row['category_name']."(".$row['sequential_order'].")"."(".$row['relevance_order'].")";
                $sid = $row['id'];
                
                if($sid!=$prev_sid)
                {
                    if($sname==$prev_sname)
                        echo "<td class='inner'>$sname</td>";
                    else
                    {
                        echo "<td>$sname</td>";
                        $prev_email = '--------'; 
                    ...
                    if($email==$prev_email)
                        echo "<td class='inner'>$name</td><td class='inner'>$surname</td><td class='inner'>$email</td><td class='inner'>$rated</td>";
                    else
                ...
                {
                    echo "<td class='inner'>$sname</td><td class='inner'>$name</td><td class='inner'>$surname</td><td class='inner'>$email</td><td class='inner'>$rated</td>".
                         "<td class='inner'>$aims</td><td class=''>$category</td><td class='inner'>$click</td><td class='inner'>$date</td>";
                }
                $prev_sname = $sname; 
                $prev_email = $email; 
                $prev_sid = $sid;
