...
<html>
    ...
    <body>
        ...
        <table>
            <thead>
                <tr><th>Stakeholder Category</th><th>Search Aim</th><th>Date</th></tr>
            </thead>
            <tbody>
        <?php
            $query = " SELECT s.order, s.name as sname, u.name, u.surname, u.email, us.aims, us.start_date FROM usersessions us, user u, stkhcategories s, rel_users_stkhcategories ustk" .
                     " WHERE s.id = ustk.stkhcategories_id AND ustk.users_id = u.id AND us.users_id=u.id AND us.start_date > '2013-10-31 00:00:00'".
                     " UNION " .
                     " SELECT -1 as `order`, 'uncategorized' as sname, u.name, u.surname, u.email, us.aims, us.start_date FROM usersessions us, user u " .
                     " WHERE NOT EXISTS ( SELECT * FROM rel_users_stkhcategories ustk where ustk.users_id = u.id) AND us.users_id=u.id AND us.start_date > '2013-10-31 00:00:00'".
                     " ORDER BY `order`, start_date desc";
             
             $query = "SELECT sess.*, count(*) as visited_links FROM " .
                      " (SELECT us.id, s.order, s.name as sname, u.name, u.surname, u.email, us.aims, us.start_date " .
                      " FROM usersessions us, user u, stkhcategories s, rel_users_stkhcategories ustk" .
                      " WHERE s.id = ustk.stkhcategories_id AND ustk.users_id = u.id AND us.users_id=u.id AND us.start_date > '2013-10-31 00:00:00'".
                      " UNION " .
                      " SELECT us.id, -1 as `order`, 'uncategorized' as sname, u.name, u.surname, u.email, us.aims, us.start_date FROM usersessions us, user u " .
